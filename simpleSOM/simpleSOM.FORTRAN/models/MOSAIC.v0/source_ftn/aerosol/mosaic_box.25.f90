...
!   subr arguments
	...
	iprint_mosaic = iprint
      ...
      !PRINT*, 'HERE in MOSAIC.BOX'
      !PRINT*, cnn(noffset + knum_a)
...
! subr arguments
      real(r8) :: dtchem
! local variables
      integer ibin, isize, itype, iv, nOFFSET
      real(r8) :: XT
...
!!!!!! CHARLES
         ...
         !IF (cnn(noffset + knum_a).GT.0.) THEN
         !   jaerosolstate(ibin) = 1
         !END IF

         !IF (iBIN.EQ.1) THEN
         !PRINT*, 'HERE in MOSAIC.SOLVER'
         !PRINT*, num_a(1)
         !END IF
...
!!! MARK
       ...
       enddo
        ...
        !PRINT*, 'HERE in MAP.MOSAIC.BOX.IN'
        !PRINT*, aer(:,jtotal,1)
        !PRINT*, num_a
        !READ(*,*)
...
! gas
        ...
        !PRINT*, 'HERE in MAP.MOSAIC.BOX.OUT'
        !PRINT*, num_a
        !READ(*,*)
...
! in this case, num_a has probably not yet been initialized, so do it
            num_a(ibin) = vol_dry_a(ibin)/volume_bin(ibin)	! #/cc(air)
               ...
               !PRINT*, iBIN,num_a(iBIN),num_at_dlo,num_at_dhi
               !READ(*,*)
...
! update: oct 2005
!-----------------------------------------------------------------------
      SUBROUTINE UPDATE_BIN_DIAM(iBIN)
        
      USE mod_MAIN, ONLY:  piover6, third,nCOMP
      USE module_data_mosaic_aero
...
! Box-model only
      ...
      RETURN
      END SUBROUTINE UPDATE_BIN_DIAM
...
!      include 'v33com2'
      USE mod_MAIN, ONLY: ipmcmos,nCOMP
      USE mod_GAS,  ONLY: VOC_MW 
      use module_data_mosaic_aero

      implicit none

! local variables
      integer iaer, je, ja, j_index, ibin,j
      logical first
      save first
      data first/.true./
...
!!!   EDITED BY CHARLES !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      DO j = 1,nCOMP
         mw_gas_mac(icn3_g+j-1) = VOC_MW + (nCOMP - j)*14.
      END DO
...
!!!   EDITED BY CHARLES !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      DO j = 1,nCOMP
         mw_aer_mac(icn3_a+j-1) = VOC_MW + (nCOMP - j)*14.
      END DO
...
!!!!! EDITED BY CHARLES
      ...
      DO j = 1,nCOMP
         mw_comp_a(jcn3+j-1) = VOC_MW + (nCOMP - j)*14.
      END DO
