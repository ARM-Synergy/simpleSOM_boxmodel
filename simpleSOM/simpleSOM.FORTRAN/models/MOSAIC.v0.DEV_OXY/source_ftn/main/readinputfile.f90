...
! input_version = 2 is does input of new variables for sectional aerosol
      ...
      OPEN(UNIT=789,FILE='inputs/in.VOC',STATUS='old')
      READ(789,*) VOC,VOC_MW,VOC_kOH,VOC_CSAT
      CLOSE(789)

      OPEN(UNIT=789,FILE='inputs/in.VOC_COEFF',STATUS='old')
      READ(789,*) VOC_COEFF
      CLOSE(789)

      OPEN(UNIT=789,FILE='inputs/in.VOC_OXY',STATUS='old')
      READ(789,*) VOC_OXY
      CLOSE(789)

      OPEN(UNIT=789,FILE='inputs/in.OH',STATUS='old')
      READ(789,*) AX1,BX1,AX2,BX2
      CLOSE(789)
      ...
      if (input_version == 1) then
         ...
         ALLOCATE(k_DIAM_DRY(nBINS))
         ALLOCATE(k_NUM(nBINS))

         ALLOCATE(jk_PARMOLE_SOM(nCOMP,nBINS))
         ALLOCATE(jk_PARMOLE_SOM_2(nCOMP,nBINS))
         
         
         ALLOCATE(jk_PARMOLE_OXY(nCOMP,nBINS))
         

         ALLOCATE(jk_OLGMOLE_SOM(nCOMP,nBINS))

         k_DIAM_DRY(:) = 0.d0
         k_NUM(:) = 0.d0

         jk_PARMOLE_SOM(:,:) = 0.d0
         jk_PARMOLE_SOM_2(:,:) = 0.d0
...
!      STOP
      ...
      OPEN(UNIT=345,FILE='inputs/input.b',STATUS='old')
      READ(345,*) HETCHEM_IN
      READ(345,*) OLIG_IN
      READ(345,*) ko_f_in
      READ(345,*) ko_d_in
