...
!     dlogDp = 1.0		 ! monodisperse
      ...
      OPEN(UNIT=789,FILE='outputs/out.VOC',STATUS='UNKNOWN',ACCESS='APPEND')
      WRITE(789,'(2(E15.6))') time_dum,VOC
      CLOSE(789)
...
30    continue
      ...
      OPEN(UNIT=747,FILE='outputs/out.O2C',STATUS='unknown',ACCESS='append')
      WRITE(747,'(F15.4,5E15.5)') time_dum,SUM(jk_PARMOLE_SOM),SUM(jk_PARMOLE_OXY), &
                                  SUM(jk_PARMOLE_OXY)/SUM(jk_PARMOLE_SOM)/10., SUM(j_GASMOLE_OXY)/SUM(j_GASMOLE_SOM)/10.
      CLOSE(747)

      !PRINT*, 'HERE in PRINT'
      !PRINT*, time_dum*3600.,anum_tot
      !READ(*,*)
      
      OPEN(UNIT=747,FILE='outputs/out.NCONC',STATUS='unknown',ACCESS='append')
      WRITE(747,'(F15.4,E15.5)') time_dum, anum_tot
      CLOSE(747)
