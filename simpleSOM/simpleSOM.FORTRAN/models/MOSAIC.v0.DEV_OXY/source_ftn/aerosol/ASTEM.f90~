...
!-----------------------------------------------------------------------
      ...
      SUBROUTINE ASTEM_SOA(mdt)

      USE mod_MAIN
      USE mod_GAS, ONLY: j_GASMOLE_OXY, j_GASMOLE_SOM
      use module_data_mosaic_aero
...
! local variables
      integer ibin, iv, jp, ieqblm, nsteps_max
      
      integer iforce_eqblm
      parameter(nsteps_max = 400)
      
      real(r8) :: dtmax, t_new, t_old, t_out
      real(r8) :: sum1, sum2, sum3, sumkg, gas_save, decay_iv, delta_iv
      ...
      REAL(R8) :: j_TOTMOLE_OXY(nCOMP)
      
      REAL(R8) :: j_GASMOLE_OXY_NEXT(nCOMP)

      REAL(R8),ALLOCATABLE :: jk_PARMOLE_OXY_NEXT(:,:)

      ALLOCATE(jk_PARMOLE_OXY_NEXT(nCOMP,nBINS))
...
! overall integration loop begins over mdt seconds
...
!!!!  BY CHARLES
      DO j = 1,nCOMP
         j_TOTMOLE_OXY(j) = j_GASMOLE_OXY(j)

         DO k = 1,nBINS
            j_TOTMOLE_OXY(j) = &
            j_TOTMOLE_OXY(j) + jk_PARMOLE_OXY(j,k)
         END DO
      END DO      
...
! compute new fluxes
      ...
      DO 501 k = 1,nBINS
        !IF (jaerosolstate(k).EQ.no_aerosol) GOTO 501

        CALL ASTEM_FLUX_SOA(k)
...
! calculate maximum possible internal time-step
!11    call ASTEM_dtmax_soa(mdt, dtmax)
      t_new = t_old + dtmax	! update time
      if(t_new .gt. t_out)then	! check if the new time step is too large
        dtmax = t_out - t_old
        t_new = t_out*1.01
...
! do internal time-step (dtmax) integration
      ...
      ! right now ngas_volatile is the number for the highest volatility bin.
	    ...
	    sum1 = sum1 + numr/denr
            ...
            numr = jk_PARMOLE_OXY(iv-icn3_g+1,iBIN)
            denr = 1. + dtmax*(integrate(iv,jp,ibin)*kg(iv,ibin)*Heff(iv,ibin)/QQ(iv,ibin) + kc_firstorder(iv))
            sum3 = sum3 + numr/denr
...
!!!     ADDED BY CHARLES
        IF ((iv-icn3_g+1).LE.nCOMP) THEN
        !IF (j_GASMOLE_SOM(iv-icn3_g+1).GT.1e3) THEN
           j_GASMOLE_OXY_NEXT(iv-icn3_g+1) = (j_TOTMOLE_OXY(iv-icn3_g+1) - (1.0 + dtmax*kc_firstorder(iv))*sum3)/   &
                                             (1.0 + (1.0 + dtmax*kc_firstorder(iv))*dtmax*sum2)
        !ELSE
        !   j_GASMOLE_OXY_NEXT(iv-icn3_g+1) = j_GASMOLE_OXY(iv-icn3_g+1)
        !END IF
        END IF
! now update aer concentration in the jp phase
        do 22 ibin = 1, nbin_a

          if (jaerosolstate(ibin) .eq. no_aerosol) goto 22 !!!!! CHARLES HE
...
!!!!      ADDED BY CHARLES
          IF ((iv-icn3_g+1).LE.nCOMP) THEN
          !IF (j_GASMOLE_SOM(iv-icn3_g+1).GT.1e3) THEN
            jk_PARMOLE_OXY_NEXT(iv-icn3_g+1,iBIN) = &
            (jk_PARMOLE_OXY(iv-icn3_g+1,iBIN) + dtmax*kg(iv,ibin)*integrate(iv,jp,ibin)*j_GASMOLE_OXY_NEXT(iv-icn3_g+1))/ &
               (1. + dtmax*(integrate(iv,jp,ibin)*kg(iv,ibin)*Heff(iv,ibin)/QQ(iv,ibin) + kc_firstorder(iv)))
          !ELSE
          !  jk_PARMOLE_OXY_NEXT(iv-icn3_g+1,iBIN) = jk_PARMOLE_OXY(iv-icn3_g+1,iBIN)
          !END IF
          END IF
...
!!!!  ADDED BY CHARLES - UPDATE ARRAYS
         
      DO j = 1,nCOMP
         j_GASMOLE_OXY(j) = j_GASMOLE_OXY_NEXT(j)

         DO k = 1,nBINS
            jk_PARMOLE_OXY(j,k) = jk_PARMOLE_OXY_NEXT(j,k)
         END DO
      END DO
      
      j_GASMOLE_OXY_NEXT(:) = 0.0
      jk_PARMOLE_OXY_NEXT(:,:) = 0.0

      !PRINT*, jk_PARMOLE_OXY_NEXT

      !jk_PARMOLE_OXY(:,:) = jk_PARMOLE_OXY_NEXT(:,:)

      !j_GASMOLE_OXY(:) = j_GASMOLE_OXY_NEXT(:)

      !j_TOTMOLE_OXY = j_GASMOLE_OXY + SUM(jk_PARMOLE_OXY,2)
      
      !PRINT*, j_GASMOLE_OXY
      !PRINT*, SUM(jk_PARMOLE_OXY,2)

!      PRINT*, SUM(jk_PARMOLE_OXY)/SUM(jk_PARMOLE_SOM)/10.
!      READ(*,*)
...
! local variables
      integer iv, jp, in
      real(r8) :: dum, sum_dum, sum_soa, sum_soa_only, gamma_org
...
!!!!! BY CHARLES HE
      ...
      k_PARMOLE_SOM = SUM(jk_OLGMOLE_SOM,1)
