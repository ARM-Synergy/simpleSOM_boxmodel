...
c
      ...
      do irxn = 1, nrxn
        ...
        write(21,9001) rconst(irxn)(1:nchar)
...
c
      ...
      do irxn = 1, nrxn
        ...
        write(22,9001)rateexpr(irxn)(1:nchar)
      enddo
c
c
	  ...
	  if(il .eq. npplines(isp))then
            write(23,9001) pp(isp,il)(1:nbllen(pp(isp,il)))
	  else
             write(23,9001) pp(isp,il)(1:nbllen(pp(isp,il)))//' &'
C             PRINT*, pp(ISP,il)
C             READ(*,*)
	  ...
	  if(il .eq. nddlines(isp))then
            write(23,9001)dd(isp,il)(1:nbllen(dd(isp,il)))
	  else
            write(23,9001)dd(isp,il)(1:nbllen(dd(isp,il)))//' &'
	  endif
        enddo
...
c     check for yield
      ...
      IF (CHECK.EQ.'d-') iyield_state = 0
      
C      PRINT*, dumwd
C      PRINT*, CHECK
C      PRINT*, valu(iw,il)
C      PRINT*, iyield_state
C      READ(*,*)
      
C      STOP
      
      if(iyield_state.eq.0)then
         if(valu(iw,il).GE.0.0)then !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
...
C      PRINT*, il,ir,dumline(il)(ipos(iw,il):ipos(nwords(il),il)-1)
      
      HOLDER = dumline(il)(ipos(iw,il):ipos(nwords(il),il)-1)

      IF (MOD(ctr,2).NE.0) THEN
         ...
         rconst(ir) = '      rk_bio('//irch(1:nbllen(irch))//') = '//
     &   HOLDER
      END IF
C     &        dumline(il)(ipos(iw,il):ipos(nwords(il),il)-1)
...
c load rate array: r(1), r(2), etc.
     ...
     &		          xval, nchqq, strqq, ipostn)
          irch = strqq(1)(1:nchqq(1))
          rate(ir) = 'r_bio('//irch(1:nbllen(irch))//')'
        enddo
c
c
9000  format( a )
...
C         PRINT*,praduct(iprd,irxn)
C         PRINT*,species(isp)
C     READ(*,*)

C         PRINT*, iprd,yield(iprd, irxn)
C         READ(*,*)
...
c dd strings
        ...
        if(ndd(isp).eq.0)then
           dd(isp,1)=
     &     '      d_bio('//indexx(isp)(1:nlen)//')= 0.0'
        else
           dd(isp,1)=
     &     '      d_bio('//indexx(isp)(1:nlen)//')= '//dd_rate(isp,1)
        endif
c
        do in = 2, ndd(isp)
...
c pp strings
        ...
        if(npp(isp).eq.0)then
           pp(isp,1)=
     &     '      p_bio('//indexx(isp)(1:nlen)//')= 0.0'
        else
           pp(isp,1)=
     &     '      p_bio('//indexx(isp)(1:nlen)//')= '//
c     &            pp_signn(isp,1)(1:nbllen( pp_signn(isp,1) ))//
     &            pp_yield(isp,1)(1: nbllen( pp_yield(isp,1) ))//
     &          pp_rate(isp,1)
           
C           PRINT*, isp,pp_yield(isp,1)(1: nbllen( pp_yield(isp,1) ))
C           READ(*,*)
...
c create rateexpr array: r(1) = rk(1)*s(ia)*s(ib), etc.
     ...
     &		          xval, nchqq, strqq, ipostn)
          irch = strqq(1)(1:nchqq(1))
          rk(ir) = 'rk_bio('//irch(1:nbllen(irch))//')'
